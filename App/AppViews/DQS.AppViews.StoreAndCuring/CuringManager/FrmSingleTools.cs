...
{
    public partial class FrmSingleTools : XtraForm
    {
        public FrmSingleTools()
        {
            InitializeComponent();
        }

        private void FrmSingleTools_Load(object sender, EventArgs e)
        {
            if (this.Tag != null)
            {
                GetFacilities();
        ...
        private void saveTools(int id)
        {
            using (SqlConnection conn = new SqlConnection(GlobalItem.g_DbConnectStrings))
            {
                ...
                sql = string.Format(sql,id,1, txtCuringPerson.Text,
        txtToolsType.Text,
        txtToolsPut.Text,
