...
{
    public partial class FrmUserRoleChange : StandardForm
    {
        public FrmUserRoleChange()
        {
            InitializeComponent();
        }
        private void FrmUserRoleChange_Load(object sender, EventArgs e)
        {
            LoadType("ATC_UserRoleChange", "AppStatus", cbxBillStatus);
        ...
        protected override void CustomApprove()
        {
            ...
            if (id != null && id != DBNull.Value)
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(GlobalItem.g_DbConnectStrings))
                    {
                        string sql = @"EXEC sp_EInsertUserRoleChange {0}";
