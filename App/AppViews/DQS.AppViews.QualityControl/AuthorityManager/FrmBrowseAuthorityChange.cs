...
{
    public partial class FrmBrowseAuthorityChange : StandardForm
    {
        public FrmBrowseAuthorityChange()
        {
            InitializeComponent();
        }

        private void FrmBrowseAuthorityChange_Load(object sender, EventArgs e)
        {
            LoadType("ATC_BrowseAuthorityChange", "AppStatus", cbxBillStatus);
        ...
        protected override void CustomApprove()
        {
            ...
            if (id != null && id != DBNull.Value)
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(GlobalItem.g_DbConnectStrings))
                    {
                        string sql = @"EXEC sp_EInsertRoleMenuChange {0}";
