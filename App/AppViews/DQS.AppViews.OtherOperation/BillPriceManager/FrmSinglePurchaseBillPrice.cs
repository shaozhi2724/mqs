...
{
    public partial class FrmSinglePurchaseBillPrice : DevExpress.XtraEditors.XtraForm
    {
        private int? m_id;
        public FrmSinglePurchaseBillPrice()
        {
            InitializeComponent();
        }

        private void FrmSinglePurchaseBillPrice_Load(object sender, EventArgs e)
        {
        ...
        private void txtBillCode_PopupClosing(object sender, EventArgs e)
        {
            if (this.txtBillCode.EditData != null)
            {
                ...
                object id = dataRow["采购单ID"].ToString();
                ...
                txtHistoryCode.Text = "TJ" + dataRow["采购单号"].ToString() + (entities.Count + 1).ToString().PadLeft(2, '0');

                if (id != null)
                {
                    //根据订单设置表格数据
                    this.popupGrid.SetGridData("vw_AllPurchaseDetail", "采购单ID", id, true);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    XtraMessageBox.Show("调价单号已存在，请检查并重新选择采购记录。", "错误", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
                ...
                if (data.Rows.Count > 0)
                {
                    ...
                    approveEntity.ApproveTitle = string.Format("采购调价(正常)，编号：{0}", entity.BillPriceHistoryCode);
                    approveEntity.ApprovalContent = String.Format("采购调价(正常) {0} 申请审批。", entity.BillPriceHistoryCode);
        ...
        protected virtual void CustomSetEntity(BUSBillPriceHistoryEntity entity)
        {
            ...
            if(null != dataRow)
            {
                entity.BillID = Convert.ToInt32(dataRow["采购单ID"]);
