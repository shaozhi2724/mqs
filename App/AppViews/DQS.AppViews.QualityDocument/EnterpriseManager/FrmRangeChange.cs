...
{
    public partial class FrmRangeChange : StandardForm
    {
        public FrmRangeChange()
        {
            InitializeComponent();
        }

        private void FrmRangeChange_Load(object sender, EventArgs e)
        {
            LoadType("BFI_RangeChange", "AppStatus", cbxBillStatus);
        ...
        protected override void CustomApprove()
        {
            ...
            if (id != null && id != DBNull.Value)
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(GlobalItem.g_DbConnectStrings))
                    {
                        string sql = @"EXEC sp_EInsertRangeChange {0}";
