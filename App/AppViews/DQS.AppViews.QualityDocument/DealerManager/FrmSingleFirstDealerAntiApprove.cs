...
namespace DQS.AppViews.QualityDocument.DealerManager
{
    public partial class FrmSingleFirstDealerAntiApprove : DevExpress.XtraEditors.XtraForm
    {
        ...
        public FrmSingleFirstDealerAntiApprove()
        {
            InitializeComponent();
        ...
        private void FrmSingleFirstDealerAntiApprove_Load(object sender, EventArgs e)
        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (this.m_id != null)
            {
                ...
                needComparedFields.Add("PurchasePriceListID", layControl.GetItemByControl(cbxPrice).Text);
                needComparedControls.Add("PurchasePriceListID", cbxPrice.Name);
                ...
                if (!compareResult.AreEqual || certificateChanged)
                {
                        ...
                        XtraMessageBox.Show("确定提交信息变更请求？", "确认", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
                    {
                        ...
                        antiApproveEntity.AntiApproveTitle = string.Format("{0} 于 {1} 申请变更首营供应商信息",
                            requestPerson, DateTime.Now.ToString("yyyy年M月d日 HH时m分"));
                        antiApproveEntity.DocumentCode = "FirstDealer";
