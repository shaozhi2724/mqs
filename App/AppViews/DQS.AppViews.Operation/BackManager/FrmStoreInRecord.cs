...
namespace DQS.AppViews.Operation.BackManager
{
    public partial class FrmStoreInRecord : DevExpress.XtraEditors.XtraForm
    {
        ...
        string productName;
        string batchNo;
        public FrmStoreInRecord(int _productID,string _productName,string _batchNo)
        {
            InitializeComponent();
        ...
        private void FrmStoreInRecord_Load(object sender, EventArgs e)
        {
            ...
            DataTable data = GlobalMethod.GetStoreInByProduct(productID, batchNo);
        ...
        private void gvDetails_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                int selectedProductID = (int)EditRow["药品ID"];
                string selectedProductName = EditRow["药品名称"].ToString();
                string selectedBatchNo = EditRow["批号"].ToString();
                var selectedPackageSpec = EditRow["包装规格"].ToString();

                if (selectedProductID != productID)
                {
                    XtraMessageBox.Show("药品名称不匹配，请选择: " + productName, "错误", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
