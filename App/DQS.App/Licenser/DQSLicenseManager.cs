...
{
    public class DQSLicenseManager
    {
        /// <summary>
        /// 枚举注册文件，注册类型包含:
        /// Demo - 演示版. 不检测计算机ID
        ...
        public bool IsValid(string licensePath, string featureName, string passCode, bool bThrow)
        {
            ...
            try
            {
                SenseLockKeyManager.Open();
                return true;
            }
            catch (Exception ex)
            {
                throw (new LicenseException(ex.Message));
            }
            return false;
#endif
        ...
        private LicenseInfo GetLicenseFromFile(string licensePath, string passCode, out string signature)
        {
